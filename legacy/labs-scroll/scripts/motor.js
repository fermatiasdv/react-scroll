...
import { validateContent, applyCssVariables } from './helpers.js';
import {
  setDisplayInstant,
  setDisplayStatic,
  transitionDisplay,
...
function getFragrancesOverlayEl() {
  if (!fragrancesOverlayEl) fragrancesOverlayEl = document.querySelector('.fragrances-overlay');
...
function syncPocLayersVisibility() {
  const hidden = !overlapsGroupZone(window.scrollY);
  document.documentElement.classList.toggle('poc-layers-hidden', hidden);
}
...
function collapseToFreeScroll(extraScroll = 0) {
  ...
  window.scrollTo({ top: Math.max(0, boxTops[0] + extraScroll), left: 0, behavior: 'auto' });
...
function setAnimationsEnabled(enabled) {
  ...
  document.documentElement.classList.toggle('fragrances-animated', enabled);

  if (enabled) {
    document.documentElement.classList.remove('fake-scroll-mode');
    document.documentElement.classList.remove('poc-layers-hidden');
  } else {
    disposeDisplay();
    document.documentElement.classList.add('fake-scroll-mode');
    document.documentElement.classList.add('poc-layers-hidden');
  }
...
function goToIndex(index, content) {
  ...
  window.setTimeout(() => {
    ...
    window.scrollTo({ top: boxTops[currentIndex], left: 0, behavior: 'auto' });
...
function init(config) {
  ...
  if (requestedIndex !== null) {
    window.scrollTo({ top: boxTops[0], left: 0, behavior: 'auto' });
    syncPocLayersVisibility();
  }

  // Desde la ronda 13 (2026-09-15) el listener de scroll hace lo MISMO en los dos modos: sincronizar
  // la visibilidad de las capas fijas / del botón "Show fragancies" (`syncPocLayersVisibility`). Ya
  // no hay un mecanismo de bordes fake que atender acá — en modo activado el scroll está bloqueado
  // de punta a punta del grupo y sólo lo mueve `goToIndex`; en modo desactivado el grupo es una
  // única pantalla en flujo normal y el scroll nunca se bloquea. Salir por un borde colapsa el grupo
  // (ver `collapseToFreeScroll`), no libera scroll dentro de él.
  ...
  const showButton = document.getElementById('fragrancesShowButton');
  const closeButton = document.getElementById('fragrancesCloseButton');
  ...
  // Desde la ronda 13 la ✕ y la salida por un borde son literalmente lo mismo: `collapseToFreeScroll`.
  closeButton?.addEventListener('click', () => collapseToFreeScroll());
