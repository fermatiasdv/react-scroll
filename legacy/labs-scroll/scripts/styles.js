import { CONFIG, GROW_MAX, FRAGRANCE_QUERY_PARAM } from './config.js';
import { getItemPhaseBehavior } from './helpers.js';
import { PRODUCT_CONTENT_X, PRODUCT_CONTENT_Y } from './layout.js';
 ...
 */
/**
 * Ancla de prueba sobre la botella (pedido del usuario 2026-09-11): al clickearla, lleva a
 * google.com/ con el mismo query param que usa el sitio para reflejar la fragancia actual en la
 * URL (FRAGRANCE_QUERY_PARAM, ver config.js/motor.js), con el slug de la fragancia de esta caja
 * (item.fragranceSlug, ver buildFraganciaBox/buildAlmondTestBox en content.js). Sólo para probar
 * cómo se comporta un link real saliendo de adentro del POC — no es un link real del producto.
...
function createProductLinkedCanvas(item) {
  ...
  wrapper.setAttribute('aria-label', `${item.fragranceName || 'Producto'} (demo: ir a Google)`);
