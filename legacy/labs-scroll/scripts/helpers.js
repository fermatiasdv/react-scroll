...
import { ANIMATION_BEHAVIOR, ANIMATION_WHEN, SHOW_ANIMATION, GROW_MAX, pageHeightFor } from './config.js';
...
export function applyCssVariables(config, boxCount) {
  ...
  root.setProperty('--page-height', `${pageHeightFor(boxCount, window.innerHeight)}px`);
  root.setProperty('--label-transition-ms', `${config.labelTransitionMs}ms`);
  root.setProperty('--background-transition-ms', `${config.backgroundTransitionMs}ms`);
  root.setProperty('--title-font-size', `${layout.titleFontSizePx}px`);
  root.setProperty('--ingredient-font-size', `${layout.ingredientFontSizePx}px`);
  root.setProperty('--word-fade-ms', `${config.wordFadeMs}ms`);
}
