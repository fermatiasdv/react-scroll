...
/** GLB de la botella (cuerpo con grabado horneado, tapa, pico y tubo de succión). */
const MODEL_URL = 'assets/models/bottle.glb';

/** Etiqueta diamante "BSB / PAINKILLER" (PNG con alfa) — default para toda fragancia sin entrada
 * propia en LABEL_URLS_BY_SLUG. */
const DEFAULT_LABEL_URL = 'assets/models/bottle-label.png';

/**
 * Etiquetas diamante ya generadas por fragancia, indexadas por su slug (mismo `slugify(nombre)`
 * que arma content.js para `item.fragranceSlug` — ej. "Crimson Desert" -> "crimson_desert"). Una
 * fragancia que no aparece acá usa DEFAULT_LABEL_URL. Se va sumando una entrada por cada PNG nuevo
 * que se genere (mismo formato que bottle-label.png: 1024x1024, cáscara con fondo transparente).
 */
const LABEL_URLS_BY_SLUG = {
  rebellious: 'assets/models/bottle-label-rebellious.png',
  forbidden_flower: 'assets/models/bottle-label-forbiddenflower.png',
  wonder_of_the_world: 'assets/models/bottle-label-wonderoftheworld.png',
  jagged_edge: 'assets/models/bottle-label-jaggededge.png',
  crimson_desert: 'assets/models/bottle-label-crimsondesert.png',
  glitterati: 'assets/models/bottle-label-glitterati.png',
  ecstasy: 'assets/models/bottle-label-ecstasy.png',
  epicurean: 'assets/models/bottle-label-epicurean.png',
  london_legend: 'assets/models/bottle-label-londonlegend.png',
  // Painkiller no tiene entrada: DEFAULT_LABEL_URL YA ES su etiqueta ("BSB/PAINKILLER").
};
...
function resolveLabelUrl(fragranceSlug) {
  return (fragranceSlug && LABEL_URLS_BY_SLUG[fragranceSlug]) || DEFAULT_LABEL_URL;
}
